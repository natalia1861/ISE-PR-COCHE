...
- Cada sector tiene 16 páginas de 256 bytes

IMPORTANTE: la memoria es tipo NOR, por lo tanto para escribir primero hay que borrar (poner a 0xFF).
El borrado mínimo posible es por sector (4KB).

Para escribir se usará `W25Q16_WritePage_Clean`, que guarda el valor del sector, limpia el sector completo 
...
//Funciones locales
// Inicialización
static void W25Q16_Init_SPI(void);                     // Configuración de pines y driver SPI
static void SPI_callback(uint32_t event);              // Callback al terminar transmisión SPI
static void W25Q16_Init (void);                        // Inicializa la memoria Flash

// Funciones recurrentes
static void W25Q16_WriteInstruction(uint8_t val);      // Escribe una instrucción (1 byte)
...
static void W25Q16_FastRead (uint32_t startPage, uint8_t offset, uint32_t size, uint8_t *rData);   // Lectura rápida

static void W25Q16_WritePage_Clean (uint32_t page, uint16_t offset, uint32_t size, uint8_t *data); // Escritura tras limpiar página

// Control escritura
static void write_enable (void);                       // Habilita escritura
static void write_disable (void);                      // Deshabilita escritura

static uint32_t bytesToWrite (uint32_t size, uint32_t offset); // Cálculo de bytes útiles en página
...
//Funciones locales para añadir consumos y horas en flash y recibir todos
static void addConsumption(uint8_t position, float* consumption, char* hour);       // Añade un nuevo consumo en una posición
static void getAllConsumptions(float *consumptions, float *hour);                   // Lee todos los consumos y los copia

//Funciones para Tests
static void leer_mem_entera(void);              // Lee los primeros 50 bytes para depuración
...
/**
 * @brief Hilo principal de gestión de la memoria Flash. Procesa comandos mediante cola.
...
/**
 * @brief Borra la memoria que será utilizada para guardar consumos.
 */
...
/**
 * @brief Inicializa los pines y configuración del SPI para la memoria Flash W25Q16.
*/
...
/**
 * @brief Callback llamado al finalizar una operación SPI.
...
/**
 * @brief Escribe una instrucción de un solo byte en la memoria (útil para comandos simples).
 * @param val Instrucción a enviar.
 */
 ...
 * @param number_id Número de ID a leer.
...
/**
 * @brief Lee datos de la memoria Flash desde una dirección concreta.
 * @param startPage Página inicial (cada página = 256 bytes).
 * @param offset Desplazamiento desde el inicio de la página.
 * @param size Número de bytes a leer.
 * @param rData Puntero al buffer donde se guardarán los datos leídos.
 * 
...
/**
 * @brief Lee datos rápidamente usando la instrucción FastRead.
 ...
 * Añade 8 dummy clocks despues de los 24 bits de address (norma del protocolo)
 */
 ...
 * @param numBlock Índice del bloque (0 a 31).
 * 
 ...
 * @param numSector Índice del sector dentro del bloque.
 */
...
/**
 * @brief Calcula cuántos bytes se pueden escribir desde un offset hasta el final de la página.
 * @param size Tamaño total de los datos a escribir.
 * @param offset Desplazamiento dentro de la página.
 * @return Número de bytes que se pueden escribir en esa página.
 */
...
/**
 * @brief Escribe datos en una página después de limpiar el sector (obligatorio para poder escribir).
 * @param page Página de inicio.
 * @param offset Desplazamiento desde el inicio de la página.
...
/**
 * @brief Añade un nuevo valor de consumo en una posición concreta de memoria.
 * @param position Posición en memoria circular (0-9).
 ...
 * @param consumptions Puntero al array de floats donde se copiarán los consumos.
 * @param hour Puntero al array donde se copiarán las horas.
 */
