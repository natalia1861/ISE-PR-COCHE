...
//Timer
osTimerId_t id_timer__GetPressure;
void timer__GetPressure_Callback (void *arg);
...
void thread__VelocityControl (void *no_argument)
{
    ...
    id_timer__GetPressure = osTimerNew(timer__GetPressure_Callback, osTimerPeriodic, NULL, NULL);
	osTimerStart(id_timer__GetPressure, VELOCITY_REFRESH);
    ...
    while(1)
    {
        ...
        #else
        flags = osThreadFlagsWait (FLAG__GET_PRESSURE, osFlagsWaitAny, osWaitForever);
        if (flags & FLAG__GET_PRESSURE)
        {
            marcha = getPedal();
...
void timer__GetPressure_Callback (void *arg)
{
    osThreadFlagsSet(id_thread__velocityControl, FLAG__GET_PRESSURE);
}
