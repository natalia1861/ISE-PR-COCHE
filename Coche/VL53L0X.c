...
bool init(VL53L0X * lidar,bool io_2v8)
{
  ...
  if (io_2v8)
  {
    uint8_t regValue = readReg(lidar, VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV);
		writeReg(lidar,VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV,readReg(lidar,(VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV) | 0x01)); // set bit 0
  }
  ...
  writeReg(lidar,0x80, 0x00);
	...
	uint8_t msrcConfigControl = readReg(lidar, MSRC_CONFIG_CONTROL);
    if (msrcConfigControl == 0xFF) {
        // Handle error: Failed to read register
        return false;
    }
  ...
  writeReg(lidar,SYSTEM_INTERRUPT_CONFIG_GPIO, 0x04);
	uint8_t gpioHvMuxActiveHigh = readReg(lidar, GPIO_HV_MUX_ACTIVE_HIGH);
    if (gpioHvMuxActiveHigh == 0xFF) {
        // Handle error: Failed to read register
        return false;
    }
...
{
	writeByte(&hi2c2,lidar->address,reg,value);
	lidar->last_status = 0;
}
...
{
  writeWord(&hi2c2,lidar->address,reg,value);
  lidar->last_status = 0;
}
...
{
  write32Bits(&hi2c2,lidar->address,reg,value);
  lidar->last_status = 0;
}
...
{
  uint8_t value;
  readByte(&hi2c2,lidar->address,reg,&value,I2CTIMEOUT);
...
{
  uint16_t value;
  readWord(&hi2c2,lidar->address,reg,&value,I2CTIMEOUT);
...
{
  uint32_t value;
  read32Bits(&hi2c2,lidar->address,reg,&value,I2CTIMEOUT);
  lidar->last_status = 0;
  return value;
}
...
{
	writeBytes(&hi2c2,lidar->address,reg,count,src);
	lidar->last_status = 0;
}
...
{
	readBytes(&hi2c2,lidar->address,reg,count,dst,I2CTIMEOUT);
	lidar->last_status = 0;
}
